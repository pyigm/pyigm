...
def calc_cgm_rho(galaxy, igm_sys, cosmo, **kwargs):
    ...
    **kwargs :
    ...
    if isinstance(igm_sys, list):
        ...
        return calc_rho(galaxy.coord, coords,
                        np.array([galaxy.z] * len(coords)), cosmo, **kwargs)
...
def get_close_galaxies(field, rho_max=300. * u.kpc, minz=0.001, maxz=None):
    ...
    if maxz is None:
        maxz = np.inf
    field.galaxies['rho'] = field.calc_rhoimpact(field.galaxies,
                                                 comoving=False)
...
def cgm_from_galaxy_igmsystems(galaxy,
    ...
    if len(match) == 0:
        if dummysys is False:
            ...
            if verbose:
                print(
                    "No IGMSystem match found. Attaching dummy IGMSystem at z={}."
                    .format(galaxy.z))
            dummysystem = IGMSystem(dummycoords, galaxy.z, vlim=None)
            dummycomp = AbsComponent(dummycoords, (10, 6), galaxy.z,
                                     [-100., 100.] * u.km / u.s)
            dummycomp.flag_N = 0
            dummyline = AbsLine(
                'NeVI 401',
...
def covering_fraction(iontable,
                      colthresh,
                      sys_attr='rho_impact',
                      bins=None,
                      returncounts=False,
