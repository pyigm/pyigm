...
def from_igmguesses_to_complist(infile,linelist=None):
    """Reads .json file generated by IGMGuesses
    and returns a list of AbsComponent objects
    ...
    for ii, key in enumerate(igmg_dict['cmps'].keys()):
        ...
        except ValueError:  # if ion species not in provided LineList
            print("Warning: Transition not in the ISM LineList. Will try others...")
            comp = None
            for h2list in ['H2']:
                try:
                    comp = AbsComponent.from_dict(idict, skip_abslines=False, chk_sep=False, chk_data=False, chk_vel=False, linelist=h2list)
                except ValueError:
                    print("Warning: Transition not in the {} LineList.".format(h2list))
                    pass
                if comp is not None:
                    break
            if comp is None:
...
def from_igmguesses_to_joebvp(infile, outfile):
    ...
    """
    comp_list = from_igmguesses_to_complist(infile)
